...
export class ContenedorTiposComponent {
  ...
  private crearFormDetalle(detalle: PresupuestoItem): FormGroup {
    return new FormGroup({
      nombre_tipo_personalizado: new FormControl(
        detalle.nombre_tipo_personalizado ?? '',
        { nonNullable: true, validators: [Validators.required] },
      ),
      monto_estimado: new FormControl(
        {
          value: detalle.monto_estimado ?? 0,
          disabled: false,
        },
        {
          nonNullable: true,
      ...
      monto_pagado: new FormControl(detalle.monto_pagado ?? 0, {
        nonNullable: true,
        validators: [Validators.required, Validators.min(0)],
      }),
    });
  ...
  private sincronizarFormulario(
    ...
    detalles.forEach((detalle) => {
      ...
      fg.valueChanges.subscribe((value) => {
        detalle.nombre_tipo_personalizado =
          value.nombre_tipo_personalizado ?? '';
  ...
  private baseTotalesPorCategoria: Record<
    number,
    Record<number, { estimado: number; pagado: number }>
  > = {};
  private presupuestosBaseActuales: Array<{
    monto_total: number;
    monto_pagado: number;
  }> = [];
  ...
  private calcularMontoTotalPresupuesto(presupuesto: PresupuestoBoda): number {
    ...
    if (Array.isArray(items) && items.length > 0) {
      return items.reduce(
        (acc: number, item: ItemPresupuesto) =>
          acc + (item?.monto_estimado ?? 0),
        0,
      );
    }
  ...
  private calcularMontoPagadoPresupuesto(presupuesto: PresupuestoBoda): number {
    ...
    if (Array.isArray(items) && items.length > 0) {
      return items.reduce(
        (acc: number, item: ItemPresupuesto) => acc + (item?.monto_pagado ?? 0),
        0,
      );
    }
            ...
            const detalles: PresupuestoItem[] = tipos.map((t) => {
              ...
              return {
                ...
                nombre_tipo_personalizado:
                  primerItem?.nombre_tipo_personalizado ?? t.nombre,
          ...
          if (existente) {
            ...
            this.presupuestoctx.postPresupuesto(basePresupuesto).subscribe({
              next: (nuevo) => {
                ...
                if (!nuevo || !nuevo.id) {
                  console.error(
                    'Error: no se recibio ID del presupuesto creado',
                  );
                  return;
                }
  ...
  private setBaseTotalesCategoria(
    categoriaId: number,
    detalles: PresupuestoItem[],
  ) {
  ...
  private recalcularResumen() {
    ...
    const baseTotal = presupuestos.reduce(
      (total, p) => total + (p.monto_total ?? 0),
      0,
    );
    const basePagado = presupuestos.reduce(
      (total, p) => total + (p.monto_pagado ?? 0),
      0,
    );
    ...
    Object.entries(detalles).forEach(([categoriaIdStr, items]) => {
      ...
      items.forEach((item) => {
        const baseItem = base[item.tipo_producto_id] ?? {
          estimado: 0,
          pagado: 0,
        };
  ...
  private guardarItemAuto(item: PresupuestoItem) {
    ...
    const presupuestoId =
      item.presupuesto_id ??
      this.presupuestoIdPorTipo.get(item.tipo_producto_id);
